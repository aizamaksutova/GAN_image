...
from GAN.utils.utils import make_train_image, make_mega_image
from GAN.datasets import CatsBase
...
class GANTrainer:
    def __init__(
        ...
        g_lr_scheduler: torch.optim.lr_scheduler.LinearLR,
        d_lr_scheduler: torch.optim.lr_scheduler.LinearLR,
        writer,
        save_dir: str,
        device: torch.device,
        config,
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch in self.test_dataloader:
                ...
                last_idx += bs
                if i == 0:
                    real_images_dataset = CatsBase(batch["img"].detach())
                    constructed_images_dataset = CatsBase(samples.detach())
                    i += 1
